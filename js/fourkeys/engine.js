...
'use strict';
    ...
    function addShip(intro) {
        ...
        // he takes his time getting here, and comes from further down, so it
        // reads as somebody arriving rather than a unit spawning
        const riseFrom = intro ? 300 : 140;
        ships.push({
            // placed where the walk on starts, not where it ends: he can be
            // drawn before stepShipMotion first moves him, and the introduction
            // always is, because both places that add him do it after that
            // frame's step
            slot, x: slotX(slot), y: slotY(slot) + riseFrom, w,
