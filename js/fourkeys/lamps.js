'use strict';
    ...
    // them lit he is open, and hits land until the first goes out again. Four
    ...
    LAB_KNOBS.push('LAMP_LVL', 'LAMP_HP', 'LAMP_N', 'LAMP_SECS', 'LAMP_W', 'LAMP_Y',
                   'LAMP_BOSS_W', 'LAMP_BOSS_Y', 'LAMP_SWEEP', 'LAMP_CLIMB');
    ...
    LAB_BOSS.lamps = {
        start(b) {
            ...
            lamp = { t: 0, x: LW / 2, pend: null, lit: 0, windows: 0, wasOpen: false,
                     lamps: Array.from({ length: n }, (_, i) => ({ u: (i + 0.5) / n, on: 0 })) };
            lampBox(b);
        },
        reset() { lamp = null; },
        update(b, dt) {
        ...
        contact(br, ball) {
            ...
            for (const l of lamp.lamps) {
                const hit = maskContact(ball, l.u * LW, LAMP_Y, 0, lw, lh, MASK, false);
    ...
    // the box the physics looks in: him and the whole row
    function lampBox(b) {
        ...
        b.x = x0; bw = x1 - x0;
        b.y = Math.min(lamp.y - h / 2, LAMP_Y - lh / 2);
        bh = Math.max(lamp.y + h / 2, LAMP_Y + lh / 2) - b.y;
    }
    ...
    function lampDraw(b) {
        ...
        if (!raw || !stone) return;
        for (const l of lamp.lamps) {
            const x = l.u * LW - lw / 2, y = LAMP_Y - lh / 2;
